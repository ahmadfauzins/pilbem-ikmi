...
Class Admin extends CI_Controller {
	...
	public function dashboard() {
		...
		// $data['username'] = $this->Admin_Model->get_data()->result();
		// $x['username'] = $this->db->get_where('auth', ['username' => $this->session->userdata('username')])->row_array();

		$data['jmlcalon']	= $this->Admin_Model->countcalon();
		$data['jmlpemilih']	= $this->Admin_Model->countpemilih();
		$data['idsekolah']	= $this->Admin_Model->idsekolah();
		$data['datapilketos'] = $this->Admin_Model->datapilketos();
		$data['vote1'] = $this->Admin_Model->datacalon();
		$data['vote2'] = $this->Admin_Model->datacalon2();
		$data['vote3'] = $this->Admin_Model->datacalon3();
		$datavote['hasil']	= $this->Admin_Model->hasilvote();
...
$this->load->view('backend/topbar');
	...
	public function cetakdaftarhadir(){
		...
		$pdf->Cell(190,7, 'Daftar Hadir Pemilihan Ketua & Wakil Ketua BEM',0,1,'C');
		$pdf->Cell(190,7, 'Tahun Periode 2020',0,1,'C');
		$pdf->Cell(10,7,'',0,1);
		...
		$pdf->Cell(115,10, '',0,0, 'L');
		$pdf->Cell(70,10, 'Ketua STMIK IKMI Cirebon',0,1, 'L');
		$pdf->Cell(10,20,'',0,1);
	...
	public function laporan(){
		...
		//HEADER LAPORAN
		$pdf->Cell(290,7, 'LAPORAN PELAKSANAAN ',0,1,'C');
		$pdf->Cell(290,7, 'PEMILIHAN KETUA & WAKIL KETUA BEM STMIK IKMI CIREBON',0,1,'C');
		$pdf->Cell(290,7, 'TAHUN PERIODE '.$data['tapel'],0,1,'C');
		$pdf->Cell(10,7,'',0,1);
		...
		$pdf->Cell(90,21, 'Universitas',1,0, 'C');
